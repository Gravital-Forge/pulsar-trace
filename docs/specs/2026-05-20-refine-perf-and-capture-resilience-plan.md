...
- **Engine-side wedge that may also have contributed to the 17-min cutoff.** Even if every `SCStream` failure now triggers a restart, the engine's run loop in `LiveRunner` could still wedge if the live-md sink or speaker-library SQLite read blocked an `await sink.appendXyz` for long enough that frames stopped reaching the WAV writer at the same wall time as live.md stopped advancing — that's consistent with what was observed on session 113001. A targeted look at the speaker-library SQLite read path in the live loop is a separate piece of work, not blocked by anything here.
  ...
  **RESOLUTION (session 2026-05-22):** This wedge was subsequently root-caused via the phase-tracker heartbeat, and the cause was *not* the live-md sink or the speaker-library SQLite read hypothesized here — it was a synchronous `whisper_full` decode in the `LiveRunner` run loop blocking the WAV write, because the recording write sat downstream of the decode on one serial loop. It was fixed by the live-pipeline decoupling work, which moves the WAV write onto a recording-safe drain that never calls whisper and runs whisper on a separate best-effort worker. See `docs/specs/2026-05-22-live-pipeline-decoupling-design.md`.
