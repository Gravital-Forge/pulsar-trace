...
/// calls and pushes decoded frames into an `AsyncThrowingStream`; the async
...
/// clean EOF terminates the stream (R75).
final class FrameDescriptorReader: @unchecked Sendable {
    ...
    init(fd: Int32, closeOnFinish: Bool) {
        ...
        let (s, c) = AsyncThrowingStream<[Float], Error>.makeStream()
        self.stream = s
        self.continuation = c
    }
    ...
    func makeFrameIterator() -> AsyncThrowingStream<[Float], Error>.AsyncIterator {
        startIfNeeded()
    ...
    private func readLoop() throws {
        ...
        while true {
            ...
            continuation.yield(try FrameProtocol.decodePayload(payload))
        }
    }
}
