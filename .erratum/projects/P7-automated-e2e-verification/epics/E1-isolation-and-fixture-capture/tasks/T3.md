...
Run: `swift test --filter MenuBarSettingsIsolation` (bare, with `dangerouslyDisableSandbox: true`)
Expected: FAIL — `MenuBarSettings` has no `overrides:` initializer /
`defaultOutputFolderURL(overrides:)` (compile error).
...
Run: `swift test --filter MenuBarSettingsIsolation` Expected: PASS — three tests.
