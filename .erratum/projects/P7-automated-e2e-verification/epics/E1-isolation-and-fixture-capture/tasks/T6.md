...
`canStopRecording` is an existing `RecordingStatus` member (`MenuBarState.swift`) — do not add
status API for a test.

- [ ] **Step 2: Run the test to verify it fails**

Run: `swift test --filter RecordingFixtureMode` (bare, with `dangerouslyDisableSandbox: true`)
Expected: FAIL — no `overrides:` parameter on `RecordingViewModel.init` (compile error).
...
Run: `swift test --filter RecordingFixtureMode` Expected: PASS — three tests.
