...
Run: `swift test --filter "RecordPlan fixtures"` (bare, with `dangerouslyDisableSandbox: true`)
...
```swift
        ...
        if let fixtures {
            ...
            captureArgs = [
                "--recording-id", recordingId,
                // … existing capture argv exactly as today (lines 56-73) …
            ]
...
Run: `swift test --filter "RecordPlan fixtures"` Expected: PASS — five tests.
