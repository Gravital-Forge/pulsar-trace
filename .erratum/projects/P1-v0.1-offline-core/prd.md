...
capability builds on, an offline transcription engine, offline speaker diarization, a post-recording
refinement pass that produces the authoritative transcript, and a persistent speaker library that
carries speaker identity across recordings. It also lays down the cross-cutting foundations: an
append-only machine-readable events log, content-safe operational logging, and a deterministic
...
*Introduces:* PT-R70, PT-R71, PT-R72, PT-R73, PT-R75, PT-R76 *Acceptance:* the same transcription
pipeline runs unchanged over a fixture, a pipe, and a socket source and terminates cleanly on each.
...
*Introduces:* PT-R9, PT-R13 *Acceptance:* transcribing a fixture yields a Markdown transcript
matching a committed snapshot, deterministically across repeated runs.
...
*Introduces:* PT-R54c, PT-R54d *Acceptance:* an interrupted download resumes to completion; a
corrupted file is detected and re-fetched.
...
*Introduces:* PT-R54e *Acceptance:* stored WAVs are 16 kHz mono Int16 and are re-readable by the
refinement pass.
...
*Introduces:* PT-R15a, PT-R17, PT-R29 *Acceptance:* a two-party recording yields distinct speaker
turns over the system stream; mic-origin speech is always attributed to the local speaker.
...
*Introduces:* PT-R20, PT-R21, PT-R24, PT-R25, PT-R26, PT-R38, PT-R39, PT-R48 *Acceptance:* refining
a recording produces a final transcript and metadata sidecar; an interrupted refine leaves no
partial transcript behind.
...
*Introduces:* PT-R22, PT-R23, PT-R28, PT-R30, PT-R32a, PT-R32b, PT-R49, PT-R83 *Acceptance:* a
returning speaker in a second recording is matched to the same stable ID and name.
...
*Introduces:* PT-R78, PT-R79, PT-R80, PT-R81, PT-R82, PT-R84, PT-R85 *Acceptance:* a refinement run
emits a causal, schema-valid sequence of events containing no content.
...
*Introduces:* PT-R57, PT-R58, PT-R59, PT-R60, PT-R61 *Acceptance:* a log content-leak test passes;
subprocess stderr is captured into the same log.
...
tests are deterministic (seeded, pinned, snapshot-asserted) and include a real IPC integration test
over the source seam.

*Introduces:* PT-R62, PT-R63, PT-R64, PT-R65, PT-R67, PT-R67a *Acceptance:* the test layers run
green independently and reproducibly.
...
*Introduces:* PT-R87 *Acceptance:* no code path transmits audio, transcript, or usage data
off-device.
...
*Introduces:* PT-R88 *Acceptance:* every shipped dependency has an open-source licence.
...
*Introduces:* PT-R89 *Acceptance:* a format change without a version bump fails review.
