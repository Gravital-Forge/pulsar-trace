...
pipeline and dominating refine time. The queued refine reached parity with the one-shot path:
speaker reconciliation, the full refinement event sequence, real paused-state handling, and
propagated (non-swallowed) errors with a persisted last-error.
...
- **PT-P3-R3 (parity/perf)** — `Sources/PulsarTraceEngine/Refinement/Jobs/SharedTranscriber.swift`,
  `ResumableRefiner.swift`
