...
- **Keep tables narrow.** `MD013` checks table rows against the 150-column limit and `mdformat`
  cannot wrap inside a table cell. If a table would exceed the limit — typically a "Requirements
  satisfied" row carrying a long file/symbol list — **rewrite it as a bullet list** instead; list
  items wrap to 100 columns and stay within the limit. Reserve tables for genuinely tabular,
  short-celled data such as the traceability matrix.
