...
/// Unit coverage of mic-echo deduplication (PT-R145).
///
/// When the user listens on speakers, the mic picks up the system audio and
/// the same words appear twice. PT-R145: a mic utterance > 0.5 similar to a system
/// utterance within ±5 s is an echo — the mic-side copy is dropped.
@Suite("Mic-echo dedup (PT-R145)")
struct MicEchoDedupTests {
    ...
    func exactlyHalfNotEcho() {
        ...
        // 0.5 similar — PT-R145 requires *strictly greater than* 0.5.
        #expect(!dedup.isMicEcho(
