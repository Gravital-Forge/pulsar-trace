...
/// Unit coverage of `WhisperTranscriber.coalesceRegions` — the gap-merge step
...
struct SpeechRegionTests {
    ...
    func emptyStaysEmpty() {
        #expect(WhisperTranscriber.coalesceRegions([], minGap: .seconds(1)).isEmpty)
    }

    @Test("a single region passes through unchanged")
    func singleRegionUnchanged() {
        let only = region(1.0, 4.0)
        let out = WhisperTranscriber.coalesceRegions([only], minGap: .seconds(1))
        #expect(out == [only])
    }
    ...
    func closeRegionsMerge() {
        ...
        let out = WhisperTranscriber.coalesceRegions(
