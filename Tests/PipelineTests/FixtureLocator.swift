...
///
/// Note: the original fixture path was written as `tests/fixtures/audio/`
/// (lowercase). On a case-insensitive macOS filesystem that collides with
/// SwiftPM's required `Tests/` directory; on a case-sensitive filesystem they
/// would be two directories. The canonical capitalized `Tests/Fixtures/` avoids
/// the collision (PT-P1-D6).
enum FixtureLocator {
